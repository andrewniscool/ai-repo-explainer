...
function handleRepository(repository: GitHubRepository) {
    ...
    <main>
      <section class="hero-section" aria-labelledby="hero-title">
        <div class="hero-content">
          ...
          <h1 id="hero-title">
            Understand <span className="highlight">any</span> Repository
          </h1>          
          <p class="hero-copy">Paste a public GitHub URL to get started.</p>
          <LinkSubmission @submit="handleRepository" />

          <Transition name="result">
            <div v-if="selectedRepository" class="repository-result" role="status">
              <span class="result-icon" aria-hidden="true">✓</span>
              <span>
                Ready to analyze
                <strong>{{ selectedRepository.owner }}/{{ selectedRepository.repository }}</strong>
              </span>
            </div>
          </Transition>
        </div>
